...
class BaseHandler(webapp2.RequestHandler):
	...
	def render_front(self, form=False, title="", blog="", error="",
		...
		self.render("main.html", form=form,	username=username,
					title=title, blog=blog,	error=error, blogs=blogs)
...
class NewPost(BaseHandler):
	...
	def post(self):
		...
		if title and blog and username:
			...
			error = "We need both a title and a blog in order to "
			+ "publish this entry."
			self.render("main.html", form=True, action="/blog/newpost",
		...
		if post.author.username == current_user.username:
			...
			if voted == "like":
				if models.Like.vote_of_post(post, current_user):
					...
					vote = models.Like
					.gql("WHERE post=:1 AND user=:2", post, current_user)
					.get()
					vote.delete()
					time.sleep(0.2)
...
class NewComment(BaseHandler):
	def post(self, number):
		...
		author = models.User
				 .gql("WHERE username=:1", self.get_current_user())
				 .get()
		post = models.Blog.get_by_id(int(number))
